...
def test_scene_detector(test_video_path, test_output_dir):
    ...
    for threshold in thresholds:
        ...
        for i, scene in enumerate(scenes[:5]):  # Print first 5 scenes
            print(f"  Scene {scene.scene_id}: {scene.start:.2f}s - {scene.end:.2f}s")
        ...
        result_file = os.path.join(test_output_dir, f"scenes_threshold_{threshold}.json")
