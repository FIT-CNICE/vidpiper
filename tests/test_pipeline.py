...
from vidpiper.core import Pipeline, PipelineStage, PipelineResult, Scene
...
class TestPipelineIntegration:
    ...
    def test_pipeline_with_custom_stage(self):
        ...
        initial_data = PipelineResult(
            ...
            scenes=[
                Scene(
                    scene_id=1, start=0.0, end=3.0
                ),  # 3 seconds (should be filtered)
                Scene(scene_id=2, start=10.0, end=20.0),  # 10 seconds
                Scene(
                    scene_id=3, start=20.0, end=24.0
                ),  # 4 seconds (should be filtered)
            ],
        )
