...
TEST_VIDEO_PATH = os.path.join(BASE_DIR, "vid/an-ai-team-lead-guide.mp4")
...
os.makedirs(os.path.join(TEST_OUTPUT_DIR, "processor_output/screenshots"), exist_ok=True)
...
def get_test_stage_status(stage_name):
    ...
    if stage_name == "scene_detector":
        ...
        return os.path.exists(os.path.join(TEST_OUTPUT_DIR, "scenes_threshold_30.0.json"))
    
    elif stage_name == "summary_generator":
        # Check if processed scenes exist
        return os.path.exists(os.path.join(TEST_OUTPUT_DIR, "processed_scenes.json"))
