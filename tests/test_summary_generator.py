...
def test_summary_generator(test_output_dir):
    ...
    processed_scenes_file = os.path.join(test_output_dir, "processed_scenes.json")
    ...
    if os.path.exists(processed_scenes_file):
        ...
        if not os.path.exists(mock_screenshot_path):
            try:
                import cv2
                import numpy as np
                cv2.imwrite(mock_screenshot_path, np.ones((480, 640, 3), dtype=np.uint8) * 255)
    ...
    try:
        generator = AnthropicSummaryGenerator(model="claude-3-7-sonnet-20240307", max_tokens=500)
