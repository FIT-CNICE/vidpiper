...
def run_single_test(test_name, force=False, provider="anthropic"):
    ...
    try:
        if test_name == "scene_detector":
            ...
            print(
                "Available tests: scene_detector, scene_processor, summary_generator"
            )
            return 1
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "test_name",
        choices=["scene_detector", "scene_processor", "summary_generator"],
    ...
    parser.add_argument(
        ...
        default="anthropic",
        help="LLM provider to use for summary generation",
    )
