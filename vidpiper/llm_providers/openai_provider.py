...
class OpenAIGenerator(LLMGenerator):
    ...
    def __init__(
        self, model: str = "gpt-4o-2024-11-20", max_tokens: int = 2000
    ):
        # Check if the 'openai' package can be found
        openai_spec = importlib.util.find_spec("openai")
        if openai_spec is None:
            # Raise error if the package is not installed
            raise ImportError(
        ...
        self.client = None  # Default client to None

        # If API key exists, import the specific class and initialize client
        if self.api_key:
            # Import the specific class only when needed
